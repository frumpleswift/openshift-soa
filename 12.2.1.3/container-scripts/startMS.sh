...
#removed logging so I can see if this actually worked in the docker logs
/u01/oracle/oracle_common/common/bin/wlst.sh -skipWLSModuleScanning /u01/oracle/dockertools/updListenAddress.py $vol_name $thehost ${MANAGED_SERVER} ${ADMIN_HOST} 

# Start SOA server
#
#I think a lot of the crap here is pushing the managed server to a background process then monitoring that process.
# This can probably be modified to simply run in the foreground
#
